...
import styles from '../../create/create-product.module.css';
...
export default function UpdateProduct({ params }: { params: { id: string } }) {
    const initialState: UpdateProductInput = {
        category: '',
        variants: [],
        product_sku: '',
        title: '',
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const res = await productsApiRequest.getSingleProduct(params.id)
                setProduct({ ...res.payload, category: res.payload.category?._id || '' })
                form.setFieldsValue({ ...res.payload, category: res.payload.category?._id || '' })
                setUploadedImages(form.getFieldValue('images'))
                setFileList(form.getFieldValue('images'))
                ...
                colorsArr.forEach(c => {
                    sizesArr.forEach(s => {
                        const inven = res.payload.variants?.find(
                            variant => s === variant.size && c === variant.color
                        )
                        if (inven)
                            inventoryMap.push({
                                color: inven.color,
                                size: inven.size,
                                quantity: inven.inventory
                            })
                    })
                })
    ...
    const handleUpdateProductSubmit = async (values: UpdateProductInput) => {
        if (!token) return;
        const variants: Variant[] = []

        colors.forEach(color => {
            sizes.forEach(size => {
                const quantity = inventory.find(item =>
                    item.color == color && item.size == size)?.quantity || 0
                if (product.variants?.find(variant =>
                    variant.color === color && variant.size === size)
                ) {
                    const old_variant = product.variants?.find(variant =>
                        variant.color === color && variant.size === size)
                    if (old_variant) variants.push({ ...old_variant, inventory: quantity })
                } else {
        ...
        try {
            ...
            await productsApiRequest.update(token, dispatch, params.id, body)
    ...
    return (
        <Form
            ...
            onFinish={(values) => handleUpdateProductSubmit(values)}
            disabled={loading}
        ...
        >
                    ...
                    },
                    {
                        validator: (_, fileList) => {
                            return new Promise((resolve, reject) => {
                                ...
                                fileList.forEach((file: RcFile | UploadedImage) => {
            ...
            <Form.Item>
                <Button htmlType='submit' type='primary'>Lưu thay đổi</Button>
            </Form.Item>
        </Form>
    );
}
